...
class ScorecardPDF(FPDF):
    ...
    def scorecard(self, row, score):
        ...
        # >= und <= statt Sonderzeichen (DejaVu unterstuetzt kein Unicode)
        metriken = {
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        status, _ = ampel(score, "Liefertreue_%") if False else \
                    ("GRÜN" if score >= 80 else "GELB" if score >= 50 else "ROT", None)
    ...
    print(f"\n🎉 Fertig – {len(df)} PDFs erstellt in /{OUTPUT_DIR}/")
